...
public class LowMemoryHashMap<K, V> implements Map<K, V>, Serializable {

	/**
	 * In the interest of memory-savings, we start with the smallest feasible
	 * power-of-two table size that can hold three items without rehashing. If
	 * we started with a size of 2, we'd have to expand as soon as the second
	 * item was added.
	 */
	...
	private class EntrySet extends AbstractSet<Entry<K, V>> {
		...
		public boolean add(Entry<K, V> entry) {
			boolean result = !LowMemoryHashMap.this.containsKey(entry.getKey());
			LowMemoryHashMap.this.put(entry.getKey(), entry.getValue());
			return result;
		}

		@Override
		public boolean addAll(Collection<? extends Entry<K, V>> c) {
			LowMemoryHashMap.this.ensureSizeFor(size() + c.size());
			return super.addAll(c);
		}

		@Override
		public void clear() {
			LowMemoryHashMap.this.clear();
		}
		...
		public boolean contains(Object o) {
			...
			V value = LowMemoryHashMap.this.get(entry.getKey());
			return LowMemoryHashMap.this.valueEquals(value, entry.getValue());
		}

		@Override
		public int hashCode() {
			return LowMemoryHashMap.this.hashCode();
		}
		...
		@Override
		public int size() {
			return LowMemoryHashMap.this.size;
		}
	}
	...
	private class KeySet extends AbstractSet<K> {
		...
		@Override
		public boolean contains(Object o) {
			return LowMemoryHashMap.this.containsKey(o);
		}
	...
	private class Values extends AbstractCollection<V> {
		...
		@Override
		public boolean contains(Object o) {
			return LowMemoryHashMap.this.containsValue(o);
		}
	...
	public LowMemoryHashMap() {
		initTable(INITIAL_TABLE_SIZE);
	}

	public LowMemoryHashMap(Map<? extends K, ? extends V> m) {
	 ...
	 * -1 if the key is not in the table. Default access to avoid synthetic
	...
	private void plugHole(int hole) {
		...
		while (keys[index] != null) {
			...
			if (hole < index) {
				 ...
				 * is from hole (exclusive) to index (inclusive).
				 */
				...
				/*
				 * "Wrapped" case, the index is before the hole (we've wrapped)
				 * and the "good range" is from index (exclusive) to hole
				 * (inclusive).
				 */
